...
inline bool isinf_helper(float v) {
  return std::isinf(v);
}
inline bool isinf_helper(c10::BFloat16 v) {
  return std::isinf(static_cast<float>(v));
}
inline bool isinf_helper(at::Half v) {
  return std::isinf(static_cast<float>(v));
}
...
void MovingAverageMinMax(
  ...
  if (*observer_on == 1) {
    if (i < size) {
      ...
      T adjusted_min = isinf_helper(running_min[i]) ? curr_min
                                                    : (running_min[i]) +
              averaging_const_t * (curr_min - (running_min[i]));

      T adjusted_max = isinf_helper(running_max[i]) ? curr_max
                                                    : (running_max[i]) +
...
struct CalcMovingAvgQparamsHelperKernelFunctor {
  void operator()(sycl::nd_item<1> item) const {
    ChooseQuantizationParamsKernelImpl<T>(
        fake_quant_on_data_,
        running_min_data_,
        running_max_data_,
        qmin_,
  ...
  CalcMovingAvgQparamsHelperKernelFunctor(
      ...
      const T* running_min_data,
      const T* running_max_data,
      int32_t qmin,
      int32_t qmax,
      int size,
 ...
 private:
  ...
  const T* running_min_data_;
  const T* running_max_data_;
  int32_t qmin_;
  int32_t qmax_;
  int size_;
